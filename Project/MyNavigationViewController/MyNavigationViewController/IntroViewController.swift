...
class IntroViewController: UIViewController {
    ...
    @IBAction func previousViewController(_ sender: Any) {
        // MARK: 함수에서 제네릭을 사용할 경우, 값을 전달 받는 변수 부분의 타입을 명시해주지않으면, 에러가 나는듯...
        // MARK: 위 방법이 마음에들지 않으면 그냥 인자로 MetaType을 받는 수 밖에없는듯...
        if let viewController = self.navigationController?.previousViewController(context: SplashViewController().self) {
